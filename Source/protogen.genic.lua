...
--// Examples \\--
Modules.print("Green", "<—>({[Attached Successful]})<——>({[Protegen.Genic Successful Loaded!]})<—>")
getgenv().Version = '1.3.9'
getgenv().TracerS = "L"
getgenv().Ntag = "P"
getgenv().Tracer = "T"
getgenv().Circle = "B"
...
local updateInterval = 0.5  -- Інтервал оновлення (у секундах)
local maxDistance = 50000000000000  -- Максимальна відстань для показу мітки (в студіях)
local highlightEnabled = true  -- Початкове значення видимості міток

-- Функція для перемикання видимості міток
local function toggleHighlight()
    highlightEnabled = not highlightEnabled

    -- Проходимо через усіх гравців і вмикаємо/вимикаємо мітки
    for _, otherPlayer in pairs(game.Players:GetPlayers()) do
        if otherPlayer ~= player then
            local character = otherPlayer.Character
            local nameTag = character and character:FindFirstChild("NameTag")
            
            if nameTag then
                nameTag.Enabled = highlightEnabled
            elseif highlightEnabled then
                -- Якщо highlightEnabled вмикається, створюємо нову мітку
...
-- Відстеження натискання клавіші P для перемикання
...
-- Функція для оновлення тексту міток
local function updateNameTags()
    if highlightEnabled then  -- Оновлення відбувається лише якщо highlightEnabled = true
        for _, otherPlayer in pairs(game.Players:GetPlayers()) do
            if otherPlayer ~= player then
                local character = otherPlayer.Character
                local head = character and character:FindFirstChild("Head")
                ...
                if head and rootPart then
                    ...
                    -- Якщо іменна мітка створена, оновлюємо текст і видимість на основі відстані
                    if nameTag then
                        if distance <= maxDistance then
                            nameTag.TextLabel.Text = otherPlayer.Name .. " [" .. math.floor(distance) .. " studs]"
...
-- Запуск оновлення міток із заданим інтервалом
